...
contract LotteryNFT is ERC721, Ownable {
    ...
    constructor() public ERC721("Niubi Lottery Ticket", "NLT") {}
...
contract NiubiLottery is LotteryOwnable, Initializable {
    using SafeMath for uint256;
    ...
    function reset() external onlyAdmin {
        ...
        uint256 burnAmount = niu.balanceOf(address(this)).sub(totalAmount);
        niu.transfer(address(0x000000000000000000000000000000000000dEaD), burnAmount);
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external inDrawingPhase {
        ...
        uint8 _maxNumber = maxNumber;
        for (uint i = 0; i < 4; i++) {
            require (_numbers[i] <= _maxNumber, 'exceed number scope');
        }
        uint256 _issueIndex = issueIndex;
        uint256 tokenId = lotteryNFT.newLotteryItem(msg.sender, _numbers, _price, _issueIndex);
        ...
        for (uint i = 0; i < keyLengthForEachBuy; i++) {
            userBuyAmountSum[_issueIndex][userNumberIndex[i]]=userBuyAmountSum[_issueIndex][userNumberIndex[i]].add(_price);
        }
        niu.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            niu.safeTransfer(address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            niu.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        niu.safeTransfer(address(msg.sender), _amount);
